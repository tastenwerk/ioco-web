...
PageDesigner.proto.render = function renderWebpage( locals, options, callback ){
  ...
  if( typeof( this.tmpl.compile ) === 'function' ){
    this.tmpl.compile( locals, options, function( err, addLocals ){
      ...
      self.parseWebbits( tmplRes, callback );
    });
  } else {
    var compiledJade = jade.compile( fs.readFileSync( this.tmpl._tmplFile ), { filename: this.tmpl._tmplFile } );
    var tmplRes = compiledJade( locals );
    this.parseWebbits( this.tmplRes, callback );
  }

}

PageDesigner.proto.processWebbit = function processWebbit( count, callback ){

  if( count >= this.$container.find('div[data-webbit-type]').length ){
    this.content = this.$container.html();
    console.log('content', this.content);
    return callback( null, this.content );
  }

  var $webbit = $(this.$container.find('div[data-webbit-type]')[count]);
  ...
  var self = this;
  this.renderWebbit( webbit, function( err, content ){
    $webbit.html( content );
    self.processWebbit( ++count, callback );
  });
 ...
 * @api private
 */
PageDesigner.proto.renderWebbit = function renderWebbit( webbit, callback ){
  ...
  ioco.web.addons[webbit.type].render( selectedContent, callback );
}
 ...
 * @api private
 */
PageDesigner.proto.parseWebbits = function parseWebbits( tmplStr, callback ){

  this.$container = $('<div/>').append( tmplStr );

  this.processWebbit( 0, callback );

}

PageDesigner.proto.getWebbitByName = function getWebbitByName( name ){
  console.log('looking for webbit ', name, 'in ', this.webbits);
