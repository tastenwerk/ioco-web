...
PageDesigner.proto.processWebbit = function processWebbit( count, locals, callback ){
  ...
  this.renderWebbit( webbit, locals, function( err, content ){
    console.log('content length is:', content.toString().length, content.toString() )
    if( content && content.toString().length > 12 )
      $webbit.html( content );
 ...
 * @api public
 */
PageDesigner.proto.initWebbits = function initWebbits( locals, callback ){
 ...
 * @api private
 */
PageDesigner.proto.parseTmpl = function parseTmpl( locals ){
